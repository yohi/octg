...
cleanup() {
  ...
  unset CLOUDFLARE_PREVIEW_API_TOKEN OCTG_PREVIEW_UPSTREAM_API_TOKEN OCTG_PREVIEW_CLIENT_KEY OCTG_PREVIEW_KEY_PEPPER OCTG_KEY_PEPPER
...
require_value OCTG_PREVIEW_UPSTREAM_API_TOKEN "${OCTG_PREVIEW_UPSTREAM_API_TOKEN:-}"
...
if [[ "$CONFIGURE_GITHUB" == true ]]; then
  ...
  require_value DENO_PREVIEW_DEPLOY_ORG "${DENO_PREVIEW_DEPLOY_ORG:-}"
  require_value DENO_PREVIEW_DEPLOY_APP "${DENO_PREVIEW_DEPLOY_APP:-}"
  require_value DENO_PREVIEW_DEPLOY_TOKEN "${DENO_PREVIEW_DEPLOY_TOKEN:-}"
  require_value DENO_PREVIEW_TOKENIZER_ENDPOINT "${DENO_PREVIEW_TOKENIZER_ENDPOINT:-}"
  require_value DENO_PREVIEW_TOKENIZER_AUTH_TOKEN "${DENO_PREVIEW_TOKENIZER_AUTH_TOKEN:-}"
  require_positive_integer DENO_PREVIEW_TOKENIZER_THRESHOLD_BYTES "${DENO_PREVIEW_TOKENIZER_THRESHOLD_BYTES:-}"
  require_positive_integer DENO_PREVIEW_TOKENIZER_TIMEOUT_MS "${DENO_PREVIEW_TOKENIZER_TIMEOUT_MS:-}"
fi
...
if [[ "$CONFIGURE_GITHUB" == true ]]; then
  ...
  set_github_secret DENO_PREVIEW_DEPLOY_TOKEN "$DENO_PREVIEW_DEPLOY_TOKEN"
  set_github_secret DENO_PREVIEW_TOKENIZER_AUTH_TOKEN "$DENO_PREVIEW_TOKENIZER_AUTH_TOKEN"
fi
