...
既存の `apps/gateway-worker/test/request-body.test.ts` により、宣言超過時の body cancellation と結果 metrics は既にカバーされているため、追加変更は不要でした。
...
後続コミット: `test(gateway-worker): verify request body caller compatibility`
