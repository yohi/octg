...
- Deno 2.9.6 の `deno run -A jsr:@deno/deploy@0.0.9904` CLI と `DENO_DEPLOY_TOKEN` を使用する。`deployctl` は Deno Deploy Classic 用のため使用しない。
...
- `master` push では検証成功後に `deno run -A jsr:@deno/deploy@0.0.9904` が staging copy (`<repo>/.deno-deploy-source`) から Production アプリへデプロイする。
