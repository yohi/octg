...
async function fetchUsage(env: Env, day: string): Promise<number> {
  const url = new URL("https://api.openai.com/v1/organization/usage/completions");
  const start = Math.floor(Date.parse(`${day}T00:00:00Z`) / 1000);
  url.searchParams.set("start_time", String(start)); url.searchParams.set("end_time", String(start + 48 * 3600)); url.searchParams.set("bucket_width", "1h"); url.searchParams.set("group_by", "model"); url.searchParams.set("limit", "24");
  ...
  const body = (await response.json()) as { data?: Array<{ buckets?: Array<{ results?: Array<{ input_tokens?: number; output_tokens?: number }> }> }> };
  return (body.data ?? []).flatMap((group) => group.buckets ?? []).flatMap((bucket) => bucket.results ?? []).reduce((sum, result) => sum + (result.input_tokens ?? 0) + (result.output_tokens ?? 0), 0);
}
...
export async function runReconciliation(env: Env, now: Date): Promise<ReconciliationReport[]> {
  ...
  for (const pool of ["STANDARD", "MINI"] as const) {
    ...
    const local = await env.DB.prepare("SELECT COALESCE(SUM(total_tokens), 0) AS total FROM requests WHERE utc_day = ? AND pool = ? AND status = 'completed'").bind(day, lower).first<{ total: number }>();
    const uncertain = await env.DB.prepare("SELECT request_id, COALESCE(reserved_tokens, 0) AS reserved FROM requests WHERE utc_day = ? AND pool = ? AND status = 'uncertain'").bind(day, lower).all<{ request_id: string; reserved: number }>();
    const localTokens = local?.total ?? 0; const openaiTokens = await fetchUsage(env, day); const difference = openaiTokens - localTokens;
    let status: "done" | "open" = uncertain.results.length === 0 ? "done" : "open";
